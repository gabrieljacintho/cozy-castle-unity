...
namespace ShinySSRR {
    ...
    public enum SkyboxReflectionMode {
        [InspectorName("Disabled")] Disabled,
        [InspectorName("Visible Sky Only")] VisibleSkyOnly,
        [InspectorName("Automatic Cubemap")] Cubemap,
        [InspectorName("User Defined Cubemap")] CustomCubemap
    }
    ...
    public enum VignetteMode {
        [InspectorName("Edge (box)")] Edge,
        [InspectorName("Radial")] Radial
    }
    ...
    public class ShinyScreenSpaceRaytracedReflections : VolumeComponent, IPostProcessComponent {
        ...
        [Serializable] public sealed class DebugViewParameter : VolumeParameter<DebugView> { }
        ...
        public ClampedFloatParameter thicknessMinimum = new ClampedFloatParameter(0.16f, 0.0f, 1f);
        ...
        [Tooltip("Clamps HDR reflection values to prevent overly bright spots.")]
        public ClampedFloatParameter hdrClamp = new ClampedFloatParameter(3f, 0.01f, 10f);
        ...
        [Serializable] public sealed class SkyboxReflectionModeParameter : VolumeParameter<SkyboxReflectionMode> { }

        [Tooltip("Controls how skybox reflections are rendered. 'Disabled' turns off skybox reflections. 'Visible Sky Only' reflects the skybox only when sky is visible on screen at the ray endpoint. '3D Cubemap' uses a captured cubemap for fallback reflections.")]
        public SkyboxReflectionModeParameter skyboxReflectionMode = new SkyboxReflectionModeParameter { value = SkyboxReflectionMode.Disabled };
        ...
        [Tooltip("If enabled, the skybox cubemap will be created with DefaultHDR color format")] 
        public BoolParameter skyboxHDR = new BoolParameter(false);

        [Tooltip("Culling Mask used while baking the skybox cubemap. 0 captures only the default skybox set in Render Settings")] 
        public LayerMaskParameter skyboxCullingMask = new LayerMaskParameter(0);
        ...
        [Tooltip("Attenuates reflections far from camera (distance in meters)")]
        public FloatParameter farCameraAttenuationStart = new FloatParameter(900);

        [Tooltip("Attenuates reflections far from camera (range in meters)")]
        public FloatParameter farCameraAttenuationRange = new FloatParameter(100);
        ...
        [Serializable] public sealed class VignetteModeParameter : VolumeParameter<VignetteMode> { }
        [Tooltip("Vignette falloff shape: Edge uses max distance to edges, Radial uses distance from center.")]
        public VignetteModeParameter vignetteMode = new VignetteModeParameter { value = VignetteMode.Edge };
        ...
        [Tooltip("Reduce blur across geometry edges (edge-aware weighting)")] 
        public BoolParameter edgeAwareBlur = new BoolParameter(false);

        [Tooltip("Show final result / debug view or compare view")]
        public DebugViewParameter debugView = new DebugViewParameter { value = DebugView.Final };

        [Tooltip("Depth multiplier for debug view")]
        public MinFloatParameter debugDepthMultiplier = new MinFloatParameter(0.1f, 0f);

        [Serializable] public sealed class DepthPrecisionParameter : VolumeParameter<DepthPrecision> { }

        [Tooltip("Depth precision of the downscaled depth buffer. Full precision provides better accuracy but uses more memory.")]
        public DepthPrecisionParameter depthPrecision = new DepthPrecisionParameter { value = DepthPrecision.Half };
        ...
        [Tooltip("In deferred and when also using reflections script, improve reflections blending. This option can be useful to combine opaque reflections with reflections over transparent objects.")]
        public ClampedFloatParameter opaqueReflectionsBlending = new ClampedFloatParameter(0f, 0f, 64f);

        [Tooltip("Enables support for transparent objects which will also be reflected. Choose by layers or add a Shiny Transparent Support script to the desired transparent objects.")]
        public BoolParameter transparencySupport = new BoolParameter(false);

        [Tooltip("Which layers are considered transparent.")]
        public LayerMaskParameter transparentLayerMask = new LayerMaskParameter(1);
        ...
        private void OnValidate() {
            ...
            farCameraAttenuationStart.value = Mathf.Max(0, farCameraAttenuationStart.value);
            farCameraAttenuationRange.value = Mathf.Max(0.001f, farCameraAttenuationRange.value);
        ...
        public void ApplyRaytracingPreset(RaytracingPreset preset) {
            switch (preset) {
                case RaytracingPreset.Fast:
                    ...
                    depthPrecision.Override(DepthPrecision.Half);
                    break;
                ...
                case RaytracingPreset.Superb:
                    ...
                    depthPrecision.Override(DepthPrecision.Full);
                    break;
